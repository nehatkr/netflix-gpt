...
const useTopRatedMovies = () => {
  ...
  const getTopRatedMovies = async () => {
    ...
    try {
      ...
      console.log('Fetching top rated movies from:', url.replace(/api_key=[^&]+/, 'api_key=***'));
      ...
      console.log('Top rated movies fetched successfully:', data.results?.length || 0, 'movies');
      dispatch(addTopRatedMovies(data.results || []));
    } catch (error) {
      console.error("Network error fetching top rated movies:", error.message);
      dispatch(addTopRatedMovies([]));
    }
  };
